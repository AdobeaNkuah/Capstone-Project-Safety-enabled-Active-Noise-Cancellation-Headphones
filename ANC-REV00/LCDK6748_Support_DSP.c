// Welch, Wright, & Morrow, 
// Real-time Digital Signal Processing, 2018

// *********************************************
// *********************************************
// Modified version to support the TMS320C6748 LCDK
// The 6748 LCDK does not have an ARM core, which
// performs some low-level initialization.
// The function Config_LCDK6748() is used to configure
// the PINMUX and PSC registers for the I2C and
// MCASP peripheral devices.
// *********************************************
// *********************************************
...
// PSC MMR definitions used in Config_LCDK6748()
#define SOC_PSC_1_REGS			(0x01E27000)	// base address
...
///////////////////////////////////////////////////////////////////////
// Purpose:   Performs low-level pinmux and power initialization
//            for LCDK6748
//
// Input:     None
//
...
// Calls:     None
//
// Notes:     With gratitude to Smita Shekar for the initial implementation and test
///////////////////////////////////////////////////////////////////////
void Config_LCDK6748()
{
	Uint32 status = 0;
    volatile Uint32 timeout = 0xFFFFFF;

    /* I2C PINMUX setup */
    status = PINMUX4 & ~0x0000ff00; // Retain the value of the PINMUX4 register, except bits pertaining to I2C0_SCL and I2C0_SDA.
    PINMUX4 = status | 0x00002200;  // Enable I2C0 SDA and I2C0 SCL lines of the PINMUX4 register.

    /* McASP PINMUX setup
      PINMUX0 modifed bits:
	   Bits 24:27 - Select Amute, 0x1
	   Bits 20:23 - Select AHCLKX, 0x1
	   Bits 16:19 - Select AHCLKR, 0x1
	   Bits 12-15 - Select AFSX, 0x1
	   Bits 8-11  - Select ASFR , 0x1
	   Bits 4-7   - Select ACLKX, 0x1
	   Bits 0-3   - Select ACLKR, 0x1 */
    status = PINMUX0 & ~0x0fffffff;// retain the bits that do not need to be modified. Bits 0-27 need to be modified.
    PINMUX0 = status | 0x01111111;

    /*PINMUX1 modified bits:
      Bits 8-11 : Selects AXR13 , 0x1
      Bits 4-7 : Selects AXR14 , 0x1 */
    status = PINMUX1 & ~0x00000ff0; //Retain bits that do not need to be modified and clear bits that need to be modified [bits 4-11 in this case]
    PINMUX1 = status | 0x00000110;

	/* Power up the McASP module */
   //Configure PSC1 module McASP to ALWAYS ON
    (*((volatile unsigned int *)(SOC_PSC_1_REGS +  (0xA00 + (7 * 4)))))= 0x00000003;  //Set 'NEXT' field MDCTL7 of PSC1 Register to 'Enable state'
    (*((volatile unsigned int *)(SOC_PSC_1_REGS +  PSC_PTCMD ))) = 0x00000001; // Set Bit 0 to '1' (ie ALWAYS ON) in the PTCMD 'Power Domain Transition Command register'

    do {
      status =  (*((volatile unsigned int *)(SOC_PSC_1_REGS +  PSC_PTSTAT )))  & PSC_PTSTAT_GOSTAT0;
    } while (status && timeout--);

    if (timeout != 0) {
    	timeout = 0xFFFFFF;
    	status = PSC_MDCTL_NEXT_ENABLE & PSC_MDCTL_NEXT;
    	do {
    		timeout--;
    	} while(timeout && ((*((volatile unsigned int *)(SOC_PSC_1_REGS +  (0x800 + (7 * 4))))) & PSC_MDSTAT_STATE) != status);
    }

    if (timeout == 0) {
        while(1) ; // stall here due to MCASP PSC init failure
    }
}
...
void DSP_Init()
{
	Config_LCDK6748();	// low-level initialization for 6748 LCDK
